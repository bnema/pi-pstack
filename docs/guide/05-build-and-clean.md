...
The [Opening a PR playbook](../../skills/poteto-mode/playbooks/opening-a-pr.md) inspects the diff before each commit and applies [`/unslop`](../../skills/unslop/SKILL.md) to the PR description and commit bodies. The cleanup removes narrating comments, unsupported guards, dead compatibility paths, and unrelated edits.
...
**Pitfall:** cleanup is not optional polish. A diff with narrating comments and defensive dead weight reads as unfinished to reviewers, and the extra code is where the next bug hides. If the diff feels padded, ask for a cleanup pass before committing, not after review calls it out.
