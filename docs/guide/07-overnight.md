...
use an available scheduler or external watcher to re-check until done. if you're truly stuck after a few hours, stop and write up why.
```
...
- The scheduler or watcher is external to pstack. The [Autonomous run playbook](../../skills/poteto-mode/playbooks/autonomous-run.md) uses it to re-check the finish condition on events or a heartbeat.
...
**Pitfall:** a duration is not a finish condition. "work on this for 4 hours" gives the agent nothing to check, and you'll wake up to four hours of motion instead of a result. Give the repeated run a predicate that can pass or fail.
