...
At the end of the run, before handing back, check that the log told the truth. Read the active transcript from `PI_SESSION_FILE`. Do not search other directories under `~/.pi/agent/sessions/`; they may contain unrelated private chats. Walk the log against what actually happened:
...
Before handing back, call `pstack_task` with `poteto: false` and a configured model from a different family when available. If only the parent model is configured, use `inherit-parent` and disclose the lack of model diversity. The reviewer reads the audit trail and active transcript, then flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.
