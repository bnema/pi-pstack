...
Call `pstack_task` once with one entry per explorer in its `tasks` array:

- `poteto`: `false`
- `model`: configured how-explorer model, otherwise `inherit-parent`
...
Spawn a single pstack child agent that explores and explains in one pass:

- `poteto`: `false`
- `model`: configured how-explainer model, otherwise `inherit-parent`
- `tools`: `["read", "grep", "find", "ls"]`
...
Once all explorers return, spawn a single pstack child agent to synthesize their findings into one coherent explanation:
...
After the explanation is complete, call `pstack_task` once with one critic per entry in the configured how-critics list. When no list exists, use one `inherit-parent` critic. A configured multi-provider list supplies model diversity.

For each critic:
- `poteto`: `false`
- `model`: one model from the configured how-critics list, or `inherit-parent`.
- `tools`: `["read", "grep", "find", "ls"]`
