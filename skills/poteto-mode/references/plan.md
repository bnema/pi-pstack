...
State your read of scope and constraints in one paragraph. Use `the available structured question tool` only for genuinely ambiguous intent (the **never-block-on-the-human** principle skill); give concrete options with each open question.
...
- Set `poteto: true` for implementation-oriented exploration. Set `poteto: false` for independent judgment.
- Pass `model` from the configured role. Omit it to inherit the active parent model.
...
If a phase creates or edits a skill, the phase instructs the implementer to follow Pi's skill documentation and use an installed skill-authoring workflow when available.
...
**Runtime.** Exercise the feature on the matching surface through the project's verification harness or an installed surface-driving skill. Browser, Electron, CLI, TUI, service, and native-mobile work must name the concrete driver. If none exists, flag that gap and consider `/skill:create-verification-skill`.
...
- a diff cleanup pass before commit and the **unslop** skill over any prose surface.
- the **show-me-your-work** skill to keep a decision trail when the plan is large enough to need an auditable record.
- the available GitHub or code-review skill after opening the PR.
