...
6. **Verify the chain from artifacts, not self-report.** `pstack_task` returns each child's final output and execution status in tool-result details, but child runs use `--no-session`. Grade chain-following from the files and evidence each candidate produced, plus the final artifact. Do not claim transcript-level evidence that the runtime did not persist. Look at which files each candidate actually opened. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
