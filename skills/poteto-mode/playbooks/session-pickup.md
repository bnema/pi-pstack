...
1. Locate the prior trail. For a local Pi session, resolve `PI_SESSION_FILE` and inspect only sibling sessions in that workspace directory. Do not scan other directories under `~/.pi/agent/sessions/`; they may contain private chats from unrelated projects. A provided session path, cloud-agent URL, or pushed branch is also valid. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
