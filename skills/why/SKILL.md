...
Before spawning investigators, list the available MCPs from Pi's available tools. Use the available-tools map when present. Otherwise inspect the MCP gateway's server list and instructions.
...
Call `pstack_task` once with all matching investigators in its `tasks` array so they run concurrently. One investigator per category lets each specialize in one tool's query vocabulary and result shape. Don't ask one agent to cover multiple MCPs.

Subagent config (each):
- `poteto`: `false`
- `model`: configured why-investigators model, otherwise `inherit-parent`
- Omit `tools` because MCP-backed investigators need the normal Pi toolset. The prompt must forbid writes and external mutations. That is a posture, not a sandbox. The source-control investigator may instead use a read-oriented allowlist that includes `bash` for Git history.
...
- `poteto`: `false`
- `model`: configured why-synthesizer model, otherwise `inherit-parent`
- Omit `tools`. The synthesizer spot-checks citations and may need MCP access; a narrow allowlist can hide those tools.
