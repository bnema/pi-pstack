...
The active Pi transcript is exposed as `PI_SESSION_FILE`. Its parent directory contains sessions for the active workspace. Never scan other directories under `~/.pi/agent/sessions/`; they may contain private chats from unrelated projects. Every session is JSONL.
...
3. Resolve `PI_SESSION_FILE`, then fan out across its sibling sessions with `pstack_task` when the corpus is large. Give each child a disjoint slice. Order candidates by modification time, search the topic first, read only relevant regions, and skip the current session plus obvious child-agent, eval, and test noise. Each child returns the same schema, one block per session: topic, user goal, decisions, open threads, struggles and corrections, and artifacts such as PRs, tickets, and branches, citing the session filename. For one or two sessions, search directly. Raw transcripts stay in child contexts; the main thread gets findings.
...
An adjacent feature or ticket stays out unless it blocks this one. When the capsule and thread lines outgrow a screen, cut detail before you cut threads. Write the brief through the **unslop** skill, cite chat findings by session filename and shared-record findings by their source (PR #, ticket ID, chat permalink, error-tracker issue), and sanitize private context before any public output.
